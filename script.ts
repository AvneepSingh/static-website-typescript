import {contactDataListType,contactDataList} from './data';
import {deleteContact} from './delete';
import {editContact} from './edit';
export {}
const verticalSpace = document.createElement("br");
let newcontactDataList:contactDataListType[] = [];
const runSearch = (searchValue:string) => {
    const query = searchValue.trim();
    console.log(contactDataList);
    newcontactDataList = [];

    contactDataList.forEach(function(element) { if (element.name.includes(query)) newcontactDataList.push(element); });
...
const createNewContact = () => {
    ...
    const newname:string = (document.getElementById('add-new-name') as HTMLInputElement).value;
    const newemail:string = (document.getElementById('add-new-email') as HTMLInputElement).value;
    const newphone:string = (document.getElementById('add-new-phone') as HTMLInputElement).value;
    const newdesg:string = (document.getElementById('add-new-desg') as HTMLInputElement).value;
...
SEARCH.addEventListener('keyup', function() { runSearch((this as HTMLInputElement).value); });
const SAVENEWONE = document.getElementById('saveNewOne');
SAVENEWONE?.addEventListener('click',function() {createNewContact();});